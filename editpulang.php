...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $query = mysqli_query($conn, "SELECT * FROM siswa_izinpulang WHERE id='$id'");
    ...
    if (isset($_POST['update'])) {
        ...
        // Perbarui rekaman
        $update_query = "UPDATE siswa_izinpulang SET 
        ...
        if (mysqli_query($conn, $update_query)) {
            echo "<script>alert('Data berhasil diperbarui'); window.location.href='siswapulang.php';</script>";
            exit();
        } else {
...
<body>
    <div class="wrap">
        <a href="siswapulang.php" class="blob-icon"><i class='bx bx-arrow-back'></i></a>
        <h2>EDIT IZIN SISWA PULANG</h2>
        <form action="" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" id="tlp" name="tlp" value="<?php echo htmlspecialchars($row['tlp']); ?>" placeholder="Telepon" onfocus="addCountryCode(this)" required>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" id="tanggal_izin" name="tanggal_izin" required>
            </div>

            <div class="form-group">
                <label for="status">Status:</label>
                <select id="status" name="status" required>
                    <option value="Izin pulang" <?php echo ($row['status'] == 'Izin pulang') ? 'selected' : ''; ?>>Izin pulang</option>
                </select>
            </div>
