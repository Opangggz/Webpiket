...
// Query insert ke tabel siswa_izinpulang
$sql1 = "INSERT INTO siswa_izinpulang (nama_siswa, jurusan, kelas, tanggal_izin, status, tlp, alasan, jk, namaguru, jabatan)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

// Prepare statement
$stmt1 = $conn->prepare($sql1);
if ($stmt1 === false) {
    die("Kesalahan saat menyiapkan pernyataan untuk siswa_izinpulang: " . $conn->error);
}

$stmt1->bind_param("ssssssssss", $nama_siswa, $jurusan, $kelas, $tanggal_izin, $status, $tlp, $alasan, $jk, $namaguru, $jabatan);

// Eksekusi query untuk siswa_izinpulang
if ($stmt1->execute()) {
    ...
    if ($stmt2->execute()) {
        ...
        if ($stmt3->execute()) {
            echo "<script>
                    ...
                    document.location.href = 'siswapulang.php';
                  </script>";
        } else {
    ...
    echo "Kesalahan saat memasukkan ke siswa_izinpulang: " . $stmt1->error;
}
